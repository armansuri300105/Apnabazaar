...
const Search = () => {
    ...
    useEffect(() => {
        const MlSearch = async () => {
            const res = await userSearchMl(text);
            console.log(res?.data?.recommendations);
            setMlProducts(res?.data?.recommendations)
        }
        MlSearch();
    },[text])
  ...
  return (
    <>
        <div className="bg-white mb-[30px] relative flex flex-col items-center mt-[100px]">
            ...
            {mlProducts.length > 0 ? <div className="feature-products w-[1200px]">
                <div className="flex justify-start">
                ...
                <div className="w-full mt-8 flex gap-4 flex-wrap justify-start">
                    {
                        (mlProducts && mlProducts.slice(0, 10).map((product, index) => {
                            return <ProductShow key={index} product={product} />
                        }))
                    }
                </div>
