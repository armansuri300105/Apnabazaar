...
const Addresses = ({ user, addNew, setaddNew, addressForm, setAddressForm, setCount, count }) => {
  const [selectedIndex, setSelectedIndex] = useState(null); // store selected index

  const handleSelectAddress = (index) => {
    if (count !== 1) return; // prevent selecting when not allowed
    ...
    <>
      {user?.addresses?.map((address, index) => (
        <div
          ...
          } flex flex-col gap-[7px] p-6 border-2 rounded-xl ${
            selectedIndex === index && count === 2 ? "border-yellow-400" : ""
          }`}
        >
      ...
      {count === 1 && (
        <button
          ...
          className="w-full h-[30px] bg-black text-white rounded-md text-[13px]"
        >
